  ...
  education: [
    {
      ...
      year: "2023",
      description: "Specialization in Digital Health Solutions and AI applications in Healthcare. Research focus on software engineering methodologies for health technology.",
      status: "completed"
    },
    {
      ...
      institution: "University Name",
      year: "2019",
      description: "Advanced studies in software architecture, distributed systems, and modern development methodologies.",
      status: "completed"
    },
    {
  ...
  projects: [
    {
      ...
      description: "Real-time healthcare analytics platform with predictive insights for patient care optimization.",
    ...
    },
    {
      ...
      description: "Modern portfolio website featuring an intelligent chatbot for visitor interaction and lead generation.",
      technologies: ["Next.js", "TypeScript", "Tailwind CSS", "Vercel"],
      link: "https://sureshmukhiya.com"
    },
    {
      name: "Open Source Development Tools",
      description: "Collection of developer productivity tools and libraries used by the open source community.",
...
export const predefinedQA = {
  "What are Suresh's technical skills?": {
    ...
    textResponse: "Here are my technical skills across different domains. I'm passionate about full-stack development with particular expertise in React, Node.js, and AI technologies."
  },
  "Tell me about Suresh's educational background": {
    type: "education", 
    data: portfolioData.education,
    textResponse: "I hold a PhD in Computer Science with specialization in Digital Health Solutions. My academic journey has been focused on bridging technology and healthcare."
  },
  "What is Suresh's professional experience?": {
    type: "experience",
    data: portfolioData.experience, 
    textResponse: "I have over 5 years of professional experience as a Senior Software Engineer, with a strong background in healthcare technology and full-stack development."
  },
  "What projects has Suresh worked on?": {
    type: "projects",
    data: portfolioData.projects,
    textResponse: "I've worked on various exciting projects ranging from AI-powered healthcare solutions to open-source development tools. Each project showcases different aspects of my technical expertise."
  },
  "What research has Suresh done?": {
    type: "research",
    data: portfolioData.research,
    textResponse: "My research focuses on AI applications in digital health and software engineering methodologies. I've published papers in peer-reviewed journals and conferences."
  },
  "Tell me about Suresh's background": {
    type: "overview",
    data: {
      summary: "I'm a PhD holder specializing in software engineering, digital health, and AI applications. I combine academic research with practical industry experience to create innovative technology solutions.",
      highlights: [
    ...
    textResponse: "I'm a technology professional who bridges the gap between academic research and practical software development, with a special focus on healthcare technology and AI applications."
  }
};
...
export async function generateResponse(query: string): Promise<{
  ...
  const keywords = {
    skills: ['skill', 'technology', 'programming', 'technical', 'expertise', 'languages', 'frameworks'],
    education: ['education', 'degree', 'phd', 'university', 'academic', 'study'],
    experience: ['experience', 'work', 'job', 'career', 'professional', 'company'],
  ...
  for (const [category, words] of Object.entries(keywords)) {
    if (words.some(word => queryLower.includes(word))) {
      const matchingQA = Object.entries(predefinedQA).find(([_, value]) => 
        value.type === category || (category === 'background' && value.type === 'overview')
      );
      
      if (matchingQA) {
        const [_, qaData] = matchingQA;
        return {
          type: 'predefined',
...
export function saveCustomQuestion(question: string, email?: string): Promise<boolean> {
...
export const suggestedQuestions = Object.keys(predefinedQA);
  ...
  // Check which sections are relevant to the query
  Object.entries(personalKnowledge).forEach(([section, content]) => {
    const sectionKeywords = {
      background: ["background", "about", "who", "biography", "intro"],
      education: [
        "education",
        "degree",
        "phd",
      ...
      research: ["research", "papers", "publications", "academic", "studies"],
      projects: ["projects", "work", "built", "developed", "created"],
      interests: ["interests", "hobbies", "passion", "likes", "enjoys"],
    };

    const keywords =
      sectionKeywords[section as keyof typeof sectionKeywords] || [];
  ...
  // If no specific match, return general background
  if (relevantSections.length === 0) {
    relevantSections.push(personalKnowledge.background);
  }
...
export async function generateResponse(query: string): Promise<string> {
  ...
  // Option 1: Using Hugging Face API (free with generous limits)
  if (process.env.NEXT_PUBLIC_HUGGINGFACE_API_TOKEN) {
    try {
      return await generateWithHuggingFace(query, context);
    } catch (error) {
      console.log("Hugging Face API failed, trying OpenAI...", error);
    }
  }

  // Option 2: Using OpenAI API (if you have credits)
  if (process.env.NEXT_PUBLIC_OPENAI_API_KEY) {
    try {
      return await generateWithOpenAI(query, context);
    } catch (error) {
      console.log("OpenAI API failed, using template response...", error);
    }
  }

  // Option 3: Fallback to template-based responses
  return generateTemplateResponse(query, context);
}

// Hugging Face Inference API implementation (free!)
async function generateWithHuggingFace(
  query: string,
  context: string
): Promise<string> {
  try {
    // Using models that are confirmed to work with Inference API (free!)
    const models = [
      "microsoft/DialoGPT-medium",
      "microsoft/DialoGPT-large",
      "facebook/blenderbot-400M-distill", // Great for conversations
      "microsoft/DialoGPT-small", // Reliable fallback
    ];

    const selectedModel = models[0]; // Using DialoGPT-medium by default (most reliable)

    // Create a simple prompt that works well with DialoGPT
    const prompt = `${query}`;

    console.log(`Using model: ${selectedModel} for query: "${query}"`);
    ...
    return (
      cleanedResponse ||
      "I apologize, but I had trouble generating a proper response. Could you rephrase your question?"
    );
  } catch (error) {
    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are an AI assistant that helps people learn about Suresh Kumar Mukhiya. 
                     Use the following context to answer questions about him. Be conversational, 
                     helpful, and accurate. If the context doesn't contain relevant information, 
...
function generateTemplateResponse(query: string, context: string): string {
  ...
  if (
    ...
    return `Based on what I know about Suresh, here are his technical skills:\n\n${personalKnowledge.skills}\n\nIs there a specific technology or skill area you'd like to know more about?`;
  }
  ...
  if (
    ...
    return `Here's information about Suresh's professional experience:\n\n${personalKnowledge.experience}\n\nWould you like to know more about any specific aspect of his experience?`;
  }
  ...
  if (
    ...
    return `Regarding Suresh's educational background:\n\n${personalKnowledge.education}\n\nFeel free to ask about his research or academic achievements!`;
  }

  if (
    queryLower.includes("research") ||
    queryLower.includes("papers") ||
    queryLower.includes("publication")
  ) {
    return `About Suresh's research work:\n\n${personalKnowledge.research}\n\nWould you like to know more about any specific research area?`;
  }
  ...
  if (
    ...
    return `Here are some of Suresh's notable projects:\n\n${personalKnowledge.projects}\n\nInterested in learning more about any particular project?`;
  }

  // Default response
  return `Here's what I can tell you about Suresh Kumar Mukhiya:\n\n${personalKnowledge.background}\n\nFeel free to ask me about his skills, experience, education, research, or projects!`;
}

// Suggested questions based on knowledge base
export const suggestedQuestions = [
  ...
  "What research has Suresh done?",
  "What projects has Suresh worked on?",
  "What are Suresh's interests and expertise areas?",
];
