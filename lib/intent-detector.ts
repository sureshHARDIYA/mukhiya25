...
const INTENT_PATTERNS: IntentPattern[] = [
  ...
  },
  {
    ...
    nlpChecks: [
      (doc) => doc.has("married") || doc.has("single") || doc.has("relationship"),
      (doc) => doc.has("(wife|husband|spouse|partner|girlfriend|boyfriend)"),
      (doc) => doc.has("(are|do) #Person (married|single|taken|committed)"),
      (doc) => doc.has("(have|got) #Person (wife|husband|partner|girlfriend|boyfriend)"),
    ],
    weight: 1.1,
  },
  {
